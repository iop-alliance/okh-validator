defmodule OkhValidatorWeb.ValidationHelpers.ManifestAuthor do
  use Phoenix.HTML
  ...
  defp show_manifest_author_status(%{status: "ok"} = manifest_author), do: check_manifest_author_validations(manifest_author.validations)
  defp show_manifest_author_status(%{status: status}), do: status

  defp check_manifest_author_validations(%{status: "ok"} = manifest_author_validations) do
    raw(
  ...
  defp check_manifest_author_validations(%{status: "error"} = manifest_author_validations) do
